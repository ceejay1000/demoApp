...
// dompurify is prefered to sanitize-html on the front-end because it's lighter
...
export default class Search {
  ...
  openOverlay() {
    ...
    // setTimeout is necessary because some browsers won't be ready on page load
    setTimeout(() => {
    ...
    if (posts.length) {
      this.resultArea.innerHTML = DOMPurify.sanitize(`<div class="list-group shadow-sm">
            <div class="list-group-item active">
              <strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} items found` : `1 item found`})</div >
              ${posts.map(post => {
      ...
      this.resultArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">No results to display</p>`
    }
